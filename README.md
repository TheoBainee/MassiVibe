...
- Stockage en **fichiers Parquet** via **Polars** (types `Categorical` optimisés), layout multi-type × multi-résolution : `data/{raw,aggregate}/{type}/{symbol}/…/{resolution}/…` (`1min` Massive, `1day` Yahoo multi-type). Migration : `myquantstore migrate-layout`.
