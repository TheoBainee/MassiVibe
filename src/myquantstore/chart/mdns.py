...
``myquantstore-chart._http._tcp.local.`` et peut être résolu via Bonjour/Avahi.

Usage typique : ``myquantstore chart --mdns`` → accessible depuis une tablette
sur le même réseau via ``http://myquantstore-chart.local:8050`` (selon le resolver
mDNS du client).
"""
...
def register_mdns(host: str, port: int, service_name: str = "myquantstore-chart") -> Zeroconf:
