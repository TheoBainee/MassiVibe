...
- un paramètre ``adjusted`` (split-adjustment) — mis à ``false`` pour stocks
  car MassiVibe stocke les prix **bruts** et applique l'ajustement split à la
  query (permet le toggle ``--no-split`` au runtime).

On normalise la réponse v2 vers le **schéma canonique** MassiVibe (champs longs)
...
def fetch_aggs_v2(
    ...
    :return: DataFrame Polars au **schéma canonique** MassiVibe (champs longs),
    ...
    if instrument.type == InstrumentType.FUTURES:
        raise ValueError(
            ...
            "utilisez massivibe.api.aggs_futures.fetch_aggs_futures."
        )

    multiplier, timespan = parse_timeframe(settings.timeframe)
    api_ticker = instrument.api_ticker

    # Pour stocks, on demande les prix NON ajustés (adjusted=false) car MassiVibe
